...
export async function generateMetadata({ params }: BookPageProps): Promise<Metadata> {
  ...
  if (!book) {
    return {
      ...
      openGraph: {
        ...
        title: 'Book Not Found – Bookmarkd',
        description: 'The book you are looking for does not exist.',
        images: [{ url: '/default-book-cover.png', width: 400, height: 600 }],
      },
      twitter: {
  ...
  const description = truncate(book.description || 'Discover this book and see what users think on Bookmarkd.');
  const author = book.author || 'Unknown Author';
  const imageUrl = book.image || '/default-book-cover.png';
  const bookUrl = `https://bookmarkd.fun/book/${encodeURIComponent(book.openLibraryId || book.id)}`;
...
export default async function BookPage({ params }: BookPageProps) {
  ...
  const structuredData = {
    ...
    url: `https://bookmarkd.fun/book/${encodeURIComponent(book.openLibraryId || book.id)}`,
    aggregateRating: {
